...
int WII_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
	...
	for (y = 0; y < 256; ++y)
	{
        ...
        y_gamma[y] = 255.0f * powf(y / 255.0f, 0.0f);
...
static int last_gamma;

void WII_UpdateGamma( int gamma )
{
	unsigned int y;

    last_gamma = gamma;
    float ratio = (float)gamma/100;
    float fgamma = 2.0 - (ratio*2.0);

    // Initialise the gamma table.
	for (y = 0; y < 256; ++y)
	{
		//y_gamma[y] = 255.0f * powf(y / 255.0f, 0.5f);
        y_gamma[y] = 255.0f * powf(y / 255.0f, fgamma);
...
static inline unsigned int CalculateY(unsigned int r, unsigned int g, unsigned int b)
{
    ...
    return last_gamma > 0 ? y_gamma[y] : y;
}
